...
import { Zap, CheckCircle2, ArrowRight, Shield, BarChart3, Mail, CreditCard, Cpu, Play, Sparkles, FileText, IndianRupee } from 'lucide-react';
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 font-sans selection:bg-accent selection:text-white relative overflow-hidden">
      ...
      <nav className="sticky top-6 z-50 flex items-center justify-between px-6 sm:px-8 py-4 max-w-5xl mx-auto rounded-3xl glass-panel shadow-lg border border-white/20 dark:border-slate-800/40 backdrop-blur-xl mt-6">
        ...
        <div className="flex items-center gap-3 sm:gap-6">
          <button 
            onClick={onExploreDemo || onLaunchSandbox}
            className="flex items-center gap-1.5 text-xs uppercase tracking-wider font-extrabold text-accent hover:text-accent-hover transition-colors"
          >
          ...
          <button 
            ...
            className="hidden md:inline-block text-xs uppercase tracking-wider font-extrabold text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white transition-colors"
          >
            Pricing
          </button>
          <button 
          ...
          <button 
            onClick={onGetStarted}
            className="text-xs uppercase tracking-wider font-black bg-gradient-to-r from-accent to-indigo-600 hover:from-accent-hover hover:to-indigo-700 text-white px-4 sm:px-5 py-2.5 rounded-2xl transition-all shadow-md shadow-accent/20 hover:scale-105 active:scale-95"
          >
            Get Started
          </button>
        </div>
      ...
      {/* Hero Section */}
      <main className="max-w-7xl mx-auto px-6 pt-24 pb-20 text-center relative z-10">
        <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-accent/10 dark:bg-accent/15 border border-accent/20 dark:border-accent/30 text-accent text-xs font-black uppercase tracking-widest mb-8">
          ...
          InvoicePro 2.0 with Instant Demo Sandbox
        </div>
        ...
        <p className="text-lg md:text-2xl text-slate-600 dark:text-slate-400 mb-12 max-w-3xl mx-auto leading-relaxed font-medium">
          Create GST-compliant invoices, track live inventory, and manage client relations in one unified glassmorphic platform. Designed for modern Indian freelancers and fast-growing enterprises.
        </p>
        ...
        <div className="flex flex-col sm:flex-row items-center justify-center gap-4 max-w-xl mx-auto mb-16">
          ...
          {(onLaunchSandbox || onExploreDemo) && (
            <button 
              onClick={onLaunchSandbox || onExploreDemo}
              className="w-full sm:w-auto flex items-center justify-center gap-2 bg-white dark:bg-slate-900 text-accent dark:text-indigo-400 border-2 border-accent/40 hover:border-accent dark:border-accent/40 px-8 py-4 rounded-2xl font-black text-lg transition-all shadow-md hover:-translate-y-1 hover:shadow-lg hover:shadow-accent/10"
            >
          ...
          <button 
            onClick={() => document.getElementById('pricing')?.scrollIntoView({ behavior: 'smooth' })}
            className="w-full sm:w-auto flex items-center justify-center gap-2 bg-transparent text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white px-6 py-4 rounded-2xl font-bold text-base transition-all"
          >
            View Pricing
          </button>
        </div>
          ...
          <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
            <div className="space-y-2">
              <div className="inline-flex items-center gap-2 text-xs font-black uppercase tracking-wider text-accent">
                ...
                <span>Instant Sandbox Available</span>
              </div>
              <h3 className="text-xl sm:text-2xl font-black text-slate-900 dark:text-white font-display">
                Want to test before signing up?
              </h3>
              <p className="text-xs sm:text-sm text-slate-600 dark:text-slate-400 max-w-xl leading-relaxed">
                Launch the interactive sandbox preloaded with simulated Indian GST data (Apex Global Technologies, Mumbai). Test invoice templates, inventory alerts, and live tax calculations instantly.
              </p>
            </div>
            ...
            <div className="flex items-center gap-3 shrink-0 w-full md:w-auto">
              {onExploreDemo && (
                <button
                  ...
                  className="flex-1 md:flex-none px-4 py-3 rounded-xl font-bold text-xs border border-slate-300 dark:border-slate-700 hover:border-accent text-slate-700 dark:text-slate-200 transition-all text-center"
                >
                  Template Tour
                </button>
              )}
              <button
                onClick={onLaunchSandbox || onGetStarted}
                className="flex-1 md:flex-none flex items-center justify-center gap-2 bg-gradient-to-r from-accent to-indigo-600 hover:from-accent-hover hover:to-indigo-700 text-white px-6 py-3 rounded-xl font-black text-xs uppercase tracking-wider transition-all shadow-md shadow-accent/20 hover:scale-105 active:scale-95"
              >
        ...
        <div className="max-w-7xl mx-auto px-6">
          ...
          <div className="grid md:grid-cols-3 gap-8 max-w-6xl mx-auto">
            ...
            <div className="p-8 rounded-3xl glass-panel border border-slate-200/10 dark:border-slate-800/20 hover:-translate-y-2 transition-all duration-300 group">
              ...
              <h3 className="text-xl font-bold text-slate-900 dark:text-white mb-3 font-display">GST & E-Way Transit</h3>
        ...
        <div className="grid md:grid-cols-3 gap-8 max-w-6xl mx-auto items-stretch">
          ...
          <div className="p-8 rounded-3xl glass-panel border border-slate-200/20 dark:border-slate-800/20 flex flex-col hover:-translate-y-2 transition-all duration-300">
            ...
            <ul className="space-y-4 mb-8 flex-1">
              {['Up to 50 invoices / mo', 'Standard GST Templates', 'Email & PDF Exports'].map((feature, i) => (
          ...
          <div className="p-8 rounded-3xl bg-slate-950 dark:bg-black border-2 border-accent/60 shadow-[0_20px_50px_rgba(79,70,229,0.25)] relative flex flex-col md:-translate-y-4 transition-all hover:-translate-y-6 duration-300">
            ...
            <ul className="space-y-4 mb-8 flex-1">
              {['Up to 200 invoices / qtr', 'All Custom Templates & Colors', 'Automated Stock Depletion', 'WhatsApp 1-Click PDF Share'].map((feature, i) => (
          ...
          <div className="p-8 rounded-3xl glass-panel border border-slate-200/20 dark:border-slate-800/20 flex flex-col hover:-translate-y-2 transition-all duration-300 relative">
            ...
            <ul className="space-y-4 mb-8 flex-1">
              {['Up to 1000 invoices / yr', 'Multi-user & Multiple Branches', 'Priority E-Way Transit Support', 'Full P&L Analytics Suite'].map((feature, i) => (
                <li key={i} className="flex items-center gap-3 text-slate-700 dark:text-slate-300 text-sm font-semibold">
                  <CheckCircle2 className="w-5 h-5 text-accent shrink-0" /> {feature}
                </li>
              ))}
      ...
      <footer className="bg-white/60 dark:bg-slate-900/60 border-t border-slate-200/40 dark:border-slate-800/40 backdrop-blur-md py-12 relative z-10">
        <div className="max-w-7xl mx-auto px-8 flex flex-col md:flex-row items-center justify-between gap-6">
          ...
          <div className="flex items-center gap-6">
            <button onClick={onExploreDemo || onLaunchSandbox} className="text-xs uppercase tracking-wider font-extrabold text-accent hover:underline">Interactive Demo</button>
